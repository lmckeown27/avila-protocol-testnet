...
const AssetDetailModal = ({ isOpen, onClose, asset, assetType }) => {
  ...
  const fetchHistoricalData = async () => {
    ...
    try {
      ...
      if (assetType === 'tradfi') {
        ...
        data = generateMockHistoricalData(asset.price, 7);
      } else {
        // For DeFi, use the existing marketData service
        // This would typically call marketDataService.getHistoricalData()
        data = generateMockHistoricalData(asset.spot_price || asset.price, 7);
      }
      ...
      setHistoricalData(generateMockHistoricalData(asset.price || asset.spot_price, 7));
    } finally {
      setIsLoading(false);
    }
  };

  const generateMockHistoricalData = (basePrice, days) => {
    const data = [];
    const now = Date.now();
