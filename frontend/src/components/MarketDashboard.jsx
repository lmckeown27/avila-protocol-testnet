...
const MarketDashboard = () => {
  ...
  const [tradFiData, setTradFiData] = useState([]);
  const [tradFiLoading, setTradFiLoading] = useState(true);
  const [tradFiError, setTradFiError] = useState(null);
  ...
  const [tradFiSortConfig, setTradFiSortConfig] = useState({ key: null, direction: 'asc' });
  const [deFiSortConfig, setDeFiSortConfig] = useState({ key: null, direction: 'asc' });
  ...
  const filteredAndSortedTradFi = useMemo(() => {
    ...
    if (tradFiSortConfig.key) {
      filtered.sort((a, b) => {
        let aVal = a[tradFiSortConfig.key];
        let bVal = b[tradFiSortConfig.key];
  ...
  const filteredAndSortedDeFi = useMemo(() => {
    ...
    if (deFiSortConfig.key) {
      filtered.sort((a, b) => {
        let aVal = a[deFiSortConfig.key];
        let bVal = b[deFiSortConfig.key];
  ...
  const handleSort = (key, type) => {
    const currentConfig = type === 'tradfi' ? tradFiSortConfig : deFiSortConfig;
    const setSortConfig = type === 'tradfi' ? setTradFiSortConfig : setDeFiSortConfig;
  ...
  const handleAssetClick = (asset, type) => {
  ...
  const renderSortIndicator = (key, type) => {
  ...
  // Render table header
  const renderTableHeader = (key, label, type) => (
    <th
      key={key}
  ...
  // Render table row
  const renderTableRow = (asset, type) => {
    const changeDisplay = getChangeDisplay(asset.change || asset.changePercent, asset.changePercent);
    const currentPrice = asset.price || asset.spot_price;
        ...
        <td className="px-4 py-4 whitespace-nowrap">
          <div className="flex items-center">
            <div className="w-8 h-8 bg-blue-100 dark:bg-blue-900 rounded-lg flex items-center justify-center text-sm font-bold text-blue-600 dark:text-blue-400">
              {(asset.symbol || asset.asset).charAt(0)}
            </div>
            <div className="ml-3">
  ...
  const renderMobileCard = (asset, type) => {
    const changeDisplay = getChangeDisplay(asset.change || asset.changePercent, asset.changePercent);
    const currentPrice = asset.price || asset.spot_price;
    const change = asset.change || 0;
    const changePercent = asset.changePercent || 0;
          ...
          <table className="hidden md:table w-full">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                {renderTableHeader('changePercent', '24h Change', 'defi')}
                {renderTableHeader('volume', '24h Volume', 'defi')}
                {renderTableHeader('marketCap', 'Market Cap', 'defi')}
              </tr>
            </thead>
            <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
              {deFiLoading ? (
