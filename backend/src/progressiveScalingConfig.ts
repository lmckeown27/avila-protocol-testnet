 ...
 * 
 * This file allows easy adjustment of asset counts for progressive scaling.
 * Start with minimal assets and gradually increase to find the optimal balance.
 * 
 * SCALING STRATEGY:
 * 1. Start with minimal assets (5 stocks, 3 ETFs, 5 crypto)
...
// ============================================================================
// CURRENT SCALING LEVEL: MINIMAL (STARTING POINT)
// ============================================================================

export const CURRENT_SCALING_LEVEL: ScalingConfig = {
  stocks: {
    count: 5,
...
// ============================================================================
// SCALING LEVELS FOR PROGRESSIVE INCREASE
// ============================================================================

export const SCALING_LEVELS = {
  MINIMAL: CURRENT_SCALING_LEVEL,
...
export function getCurrentLevel(): ScalingConfig {
  return CURRENT_SCALING_LEVEL;
}

export function getNextLevel(currentLevel: string): ScalingConfig | null {
  const levels = Object.keys(SCALING_LEVELS);
  ...
  return SCALING_LEVELS[levels[currentIndex + 1] as keyof typeof SCALING_LEVELS];
}

export function getPreviousLevel(currentLevel: string): ScalingConfig | null {
  const levels = Object.keys(SCALING_LEVELS);
  const currentIndex = levels.indexOf(currentLevel);
...
export function updateScalingLevel(newLevel: ScalingConfig): void {
  // This function would update the current scaling level
  // Implementation depends on how you want to handle configuration updates
